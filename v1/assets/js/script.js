   ...
   const app = {
      ...
      async load() {
         ...
         state.data.forEach((row, idx) => {
            ...
            // Assign Generic Class based on index (0 or 1)
            tagEl.className = `tg-p tag-p${pIndex + 1}`;
            if (pIndex === -1) tagEl.classList.add('tag-atm'); // Fallback
      ...
      ui() {
         ...
         // Form Toggles (Delegated for dynamic buttons)
         ...
         dom.img.addEventListener("click", async () => {
            ...
            if (setVal) {
               ...
               // Reload Options locally without refresh
